...
import { getLoyaltyLevel, LOYALTY_LEVELS, SEED_REVIEWS, CAFE } from '../../lib/constants';
...
export function AccountPage() {
  ...
  const load = async () => {
    if (!phone.trim()) return;
    setLoading(true);
    const data = await api.getCustomer(phone.trim());
    setCust(data);
    setLoading(false);
    if (!data) toast('Compte non trouvé', 'info');
  };

  const submitReview = async () => {
    if (!reviewForm.name || !reviewForm.text) { toast('Nom et avis requis', 'error'); return; }
    await api.createReview({ ...reviewForm, phone });
    toast('✓ Avis publié! Merci', 'success');
    setShowReview(false);
  };

  const level = cust ? getLoyaltyLevel(cust.loyaltyPoints ?? 0) : null;
  const nextLevel = level ? LOYALTY_LEVELS[LOYALTY_LEVELS.findIndex(l => l.id === level.id) + 1] : null;
  const ptsToNext = nextLevel ? nextLevel.min - (cust?.loyaltyPoints ?? 0) : 0;
  const progress  = nextLevel && level ? Math.round(((cust?.loyaltyPoints ?? 0) - level.min) / (nextLevel.min - level.min) * 100) : 100;

  return (
    <div style={{ background: '#080808', minHeight: '100vh', paddingBottom: 80 }}>
      <Navbar />
      <div style={{ padding: '24px 16px', maxWidth: 500, margin: '0 auto' }}>
        <h1 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 28, color: '#C9A84C', marginBottom: 20 }}>
          👤 Mon Compte
        </h1>
        ...
        {!cust && (
          <>
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="+212 6XX XXX XXX" type="tel" style={{ flex: 1, padding: '11px 14px', background: '#0F0F0F', border: '1px solid #1A1A1A', borderRadius: 8, color: '#F2EFE9', fontSize: 13, outline: 'none' }} />
              <button onClick={load} disabled={loading} style={{ padding: '11px 20px', background: '#C9A84C', color: '#000', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
                {loading ? '⟳' : 'Accéder'}
              </button>
            </div>
        ...
        {cust && (
          <>
            ...
            <div style={{
              ...
              borderRadius: 16, padding: '20px', marginBottom: 16, position: 'relative', overflow: 'hidden',
            }}>
              <div style={{ position: 'absolute', right: -20, top: -20, fontSize: 100, opacity: 0.04, fontFamily: 'serif' }}>✦</div>
              <div style={{ fontFamily: '"Cinzel",serif', fontSize: 11, color: '#C9A84C', letterSpacing: 3, marginBottom: 10 }}>
                ✦ LUX FIDÉLITÉ · {level?.icon} {level?.name.toUpperCase()}
              </div>
              <div style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 44, color: '#C9A84C', lineHeight: 1 }}>
                {cust.loyaltyPoints ?? 0}
              </div>
              <div style={{ fontSize: 10, color: '#8B6E2F', marginBottom: 8 }}>POINTS</div>
              <div style={{ fontSize: 14, color: '#F2EFE9', marginBottom: 12 }}>{cust.name}</div>
              {nextLevel && (
                <>
                  <div style={{ fontSize: 10, color: '#555', marginBottom: 4 }}>
                    {ptsToNext} pts pour {nextLevel.icon} {nextLevel.name}
                  </div>
                  <div style={{ height: 4, background: '#1A1A1A', borderRadius: 2, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg,#8B6E2F,#C9A84C)', borderRadius: 2, transition: 'width .8s ease' }}/>
                  </div>
                </>
              )}
            </div>
            ...
            {/* Level perks */}
            {level && (
              <div style={{ background: '#0F0F0F', border: '1px solid #1A1A1A', borderRadius: 12, padding: '14px', marginBottom: 14 }}>
                <div style={{ fontSize: 11, color: '#555', letterSpacing: 1, marginBottom: 10, textTransform: 'uppercase' }}>
                  {level.icon} Vos avantages {level.name}
                </div>
                {level.perks.map(p => (
                  <div key={p} style={{ display: 'flex', gap: 8, fontSize: 12, color: '#AAA', marginBottom: 5 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 18, color: '#F2EFE9', marginBottom: 10 }}>
                Historique des commandes
              </div>
              {(cust.orders ?? []).slice(0, 5).map((o: any, i: number) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '7px 0', borderBottom: '1px solid #111' }}>
                  <span style={{ color: '#666' }}>{o.date ?? '—'}</span>
                  <span style={{ color: '#888' }}>{o.total} MAD</span>
                  <span style={{ color: o.status === 'done' ? '#3DBE7A' : '#888' }}>{o.status}</span>
                </div>
              ))}
              {!(cust.orders?.length) && <div style={{ color: '#555', fontSize: 12 }}>Aucune commande enregistrée.</div>}
            </div>

            {/* Leave review */}
            <button onClick={() => setShowReview(!showReview)} style={{
              width: '100%', padding: '11px', background: 'rgba(201,168,76,.08)',
              border: '1px solid rgba(201,168,76,.2)', borderRadius: 8, color: '#C9A84C', cursor: 'pointer', fontSize: 13,
            }}>
              ⭐ Laisser un avis
            </button>

            {showReview && (
              <div style={{ marginTop: 12, background: '#0F0F0F', border: '1px solid #1A1A1A', borderRadius: 12, padding: '16px' }}>
                <input value={reviewForm.name} onChange={e => setReviewForm(f => ({...f, name: e.target.value}))} placeholder="Votre prénom" style={{ width: '100%', ...inputS, marginBottom: 10 }} />
                <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                  {[1,2,3,4,5].map(s => (
                    <button key={s} onClick={() => setReviewForm(f => ({...f, stars: s}))} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: s <= reviewForm.stars ? '#C9A84C' : '#333' }}>★</button>
                  ))}
                </div>
                <textarea value={reviewForm.text} onChange={e => setReviewForm(f => ({...f, text: e.target.value}))} placeholder="Votre avis…" rows={3} style={{ width: '100%', ...inputS, resize: 'none', height: 'auto', marginBottom: 10 }}/>
                <button onClick={submitReview} style={{ width: '100%', padding: '10px', background: '#C9A84C', color: '#000', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
                  Publier l'avis
                </button>
              </div>
            )}

            <button onClick={() => setCust(null)} style={{ width: '100%', padding: '10px', marginTop: 10, background: 'none', border: '1px solid #1A1A1A', borderRadius: 8, color: '#555', cursor: 'pointer', fontSize: 12 }}>
              Se déconnecter
            </button>
          </>
        )}
