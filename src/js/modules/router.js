...
(function(window) {
  ...
  function setURLView(view, replace) {
    // FIX: Use clean base — strip ?p= redirect params that cause URI loop
  ...
  function navigateToView(view, opts) {
    ...
    if (PRIVATE_VIEWS.includes(view)) {
      ...
      // Session valid → redirect to app.html
      window.location.href = 'app.html?mode=' + view;
      return;
    }
  ...
  // ── INIT ─────────────────────────────────────────────────────────
  function initRouter() {
    // FIX: Strip ?p= redirect artifact from GitHub Pages 404 before routing
    const params = new URLSearchParams(window.location.search);
    if (params.has('p') && !params.has('view')) {
      // Decode the original path from ?p= — redirect was for SPA deep link
      // Just clean the URL and show default view
      const cleanBase = window.location.origin + window.location.pathname;
      history.replaceState({}, '', cleanBase);
    }
